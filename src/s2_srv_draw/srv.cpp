...
int srv_setup(surface_data_t*pObj)//读入lut,创建着色器
{
    ...
    pObj->vertex_arry[0] = (float*)malloc(pObj->vertex_num/4*sizeof(srv_lut_t));
    pObj->vertex_arry[1] = (float*)malloc(pObj->vertex_num/4*sizeof(srv_lut_t));
    pObj->vertex_arry[2] = (float*)malloc(pObj->vertex_num/4*sizeof(srv_lut_t));
    pObj->vertex_arry[3] = (float*)malloc(pObj->vertex_num/4*sizeof(srv_lut_t));

    pObj->indices_arry =(unsigned int*)malloc(pObj->indices_num*3*sizeof(unsigned int)); 

    fp =fopen("../../src/shader/srv_lut.bin","rb");                                //导入顶点
    flag =fread(pObj->vertex_arry[0],pObj->vertex_num/4*sizeof(srv_lut_t),1,fp);         //读入顶点数据表,每次读sizeof(vertices)个，读一次
    if(flag==0) printf("import vertex failed\n");

    flag =fread(pObj->vertex_arry[1],pObj->vertex_num/4*sizeof(srv_lut_t),1,fp);         //读入顶点数据表,每次读sizeof(vertices)个，读一次
    if(flag==0) printf("import vertex failed\n");

    flag =fread(pObj->vertex_arry[2],pObj->vertex_num/4*sizeof(srv_lut_t),1,fp);         //读入顶点数据表,每次读sizeof(vertices)个，读一次
    if(flag==0) printf("import vertex failed\n");

    flag =fread(pObj->vertex_arry[3],pObj->vertex_num/4*sizeof(srv_lut_t),1,fp);         //读入顶点数据表,每次读sizeof(vertices)个，读一次
    if(flag==0) printf("import vertex failed\n");
                        ...
                        )
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VertexBuffer_ID);     //绑定顶点缓冲区对象
    glBufferData(GL_ARRAY_BUFFER, (pObj->vertex_num)/4*sizeof(srv_lut_t),(float*)pObj->vertex_arry[vertex_offset], GL_STATIC_DRAW);//创建并初始化顶点数据
...
void surroundview_init_vertices_vbo_wrap(surface_data_t *pObj)
{
	...
	for(i = 0;i < QUADRANTS;i ++)
	{
        ...
        srv_init_vertices_vbo(vaoId[i],vboId[i*2],vboId[i*2+1],pObj,i);
        data_Get = (float*)pObj->vertex_arry[i];
       /* for(int j=0;j<90;j++)
        {
            printf("page vbo get data %d %d is %f\n",i,j,data_Get[j]);
        } */
	}
